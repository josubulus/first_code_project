...
&& !empty(trim($_POST['nom'])) && !empty(trim($_POST['tel'])) && !empty(trim($_POST['mail'])) && !empty(trim($_POST['adresse'])) && !empty(trim($_POST['activite'])))
        {
          ...
          $req=$bdd->prepare('INSERT INTO entreprises(nom,tel,mail,adresse,activite,date_ajout,statut,statut_mail,interret,id_membre) VALUES (:nom,:tel,:mail,:adresse,:activite,now(),1,1,1,:id_membre)');
